        ...
        <div class="col-span-12 lg:col-span-5 hidden lg:flex flex-col justify-between p-10 bg-black text-white relative overflow-hidden border-r-4 border-black dark:border-white">
            ...
            <div class="my-auto space-y-6 relative z-10">
                <h1 class="text-3xl sm:text-4xl font-black leading-none tracking-tight text-white uppercase font-sans">
                    Portal Utama <br>
                    <span class="bg-yellow-400 text-black px-2 py-1 inline-block -rotate-1 border-2 border-black mt-2">Penyewa Kost.</span>
                </h1>
                <p class="text-xs text-gray-300 leading-relaxed font-bold">
                    Masuk ke portal penyewa untuk melacak masa sewa Anda, memeriksa tagihan bulanan, melakukan pembayaran tagihan secara online, dan mendownload nota pembayaran.
                </p>
                
                <div class="space-y-4 pt-2">
                    <div class="flex items-center gap-3.5">
                        <span class="text-lg">💳</span>
                        <span class="text-xs font-black uppercase tracking-wider text-yellow-400">Pembayaran Bulanan via Midtrans</span>
                    </div>
                    <div class="flex items-center gap-3.5">
                        <span class="text-lg">📄</span>
                        <span class="text-xs font-black uppercase tracking-wider text-yellow-400">Download Nota Pembayaran Resmi</span>
                    </div>
                    <div class="flex items-center gap-3.5">
                        <span class="text-lg">💬</span>
                        <span class="text-xs font-black uppercase tracking-wider text-yellow-400">Komunikasi Langsung via Chat</span>
                    </div>
                </div>
            </div>
        ...
        <div class="col-span-12 lg:col-span-7 p-8 sm:p-12 flex flex-col justify-center bg-white dark:bg-slate-900">
            <div class="w-full max-w-md mx-auto">
                ...
                <div class="mb-8 text-center lg:text-left">
                    <h2 class="text-3xl font-black text-black dark:text-white uppercase tracking-tight">Portal Penyewa</h2>
                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-1.5 font-bold uppercase tracking-wide">Khusus untuk Penyewa Aktif yang telah terdaftar resmi.</p>
                </div>
                ...
                <div class="mb-6">
                    <x-auth.google-button :route="route('auth.google', ['from' => 'penyewa'])" text="Masuk dengan Google" />
                </div>

                <!-- Divider -->
                <div class="relative flex items-center justify-center my-6">
                    <span class="absolute inset-x-0 h-1 bg-black dark:bg-white"></span>
                    <span class="relative bg-white dark:bg-slate-900 px-3 text-[10px] text-black dark:text-white font-black uppercase tracking-wider">ATAU MASUK SECARA MANUAL</span>
                </div>

                <form method="POST" action="{{ route('penyewa.login') }}" class="space-y-5" id="loginForm" data-testid="form-penyewa-login">
                    @csrf

                    <!-- Email or Phone Number (Credentials) -->
                    <div>
                        <label for="login" class="block text-[11px] font-black uppercase tracking-wider text-black dark:text-white mb-2">Email / Nomor WhatsApp</label>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-black dark:text-white">
                                👤
                            </span>
                            <input id="login" type="text" name="login" value="{{ old('login') }}" required autofocus placeholder="Contoh: tenant@example.com atau 081234567890"
                                ...
                                data-testid="input-penyewa-login">
                        </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            ...
                            @if (Route::has('penyewa.password.request'))
                                <a class="text-[10px] font-black uppercase tracking-wider text-black dark:text-yellow-400 hover:underline" href="{{ route('penyewa.password.request') }}">
                                    Lupa Password?
                                </a>
                            @endif
                        </div>
                    ...
                    <!-- Login Button -->
                    <div class="pt-3">
                        <button type="submit" data-testid="btn-submit-penyewa-login" class="w-full bg-yellow-400 hover:bg-yellow-300 text-black border-4 border-black dark:border-white font-black py-3.5 px-4 neo-btn-shadow neo-btn-interactive text-xs uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer">
                            <span>Masuk ke Dashboard Penyewa →</span>
                        </button>
                    </div>
                </form>
            </div>
